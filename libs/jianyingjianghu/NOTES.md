...
| 項目 | 值 |
|------|-----|
| 收藏目錄 | `/mnt/g/GameDevZ/700600_ZJMUD_ALL/zjmud-collection-master/剑影江湖` |
| mudlib 根 | `剑影江湖/剑影江湖` |
| 設定檔 | `（找不到）` |
| 匯入檔數 | 10250（15.9 MB） |
...
GBK → UTF-8 轉換 **10044** 個檔案。WASM build 只帶演算法式字集，表格式字集（GBK/BIG5）會 raise error，所以整個 lib 必須是 UTF-8。
...
| 映像 | 10248 檔 / 15.9 MB |
| 收到 | 293 行，耗時 4235 ms |
| 握手 | `ver1.0,$6$S5lGkzuSmnot/QQV$mwsfcy3JagdPIh9cznI0eepb2VfHF3NRteDDYx/WYGgOOeexdqGMydYFUExfs5usxxjZ7Z8CswG4RppJRCHke.` |
| 方言 | dmjh |
| opcode | 000 002 003 004 005 006 014 021 022 100 |
