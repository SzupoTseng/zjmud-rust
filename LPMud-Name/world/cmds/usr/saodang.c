...
int main(object me, string arg)
{
	...
	int shengyu=(3000+hhh-me->query("shimen/today"));//剩余次数为	
	if (!arg){		
	...
	if (cishu < 1 || cishu > 3000+hhh)
	return notify_fail(HIR"扫荡次数最少一次，最多也不能超过"+(3000+hhh)+"次。"NOR"\n");
	if (shengyu<cishu)
	return notify_fail(HIR"扫荡次数不能大于师门剩余次数！"NOR"\n");	
	if (me->query("shimen/today")<=1000)
	return notify_fail(HIR"请先完成本周1000次师门再来扫荡！"NOR"\n");
	if (me->is_fighting())
	return notify_fail(HIR"临阵磨枪？来不及啦。"NOR"\n");
    me->add("shimen/today",cishu);
	me->add("combat_exp",(80*cishu));
	me->add("potential",(40*cishu));
	me->add("experience", (20*cishu));
	me->add("score", (5*cishu));
	if(me->query("shen")>0)
	{
	me->add("shen", (5*cishu));
	}
	...
	me->add("weiwang", (1*cishu));	
	for(int i=1;i<=cishu;i++){	
	me->add("quest_count",1);
	...
	tell_object(me,HIG"获得了："+(80*cishu)+"经验、"+(40*cishu)+"潜能、"+(20*cishu)+"体会、"+(5*cishu)+"阅历、"+(5*cishu)+"正神、"+(1*cishu)+"威望。"NOR"\n");
	log_file("cmds/saodang", sprintf("%s：%s(%s)扫荡了，次数："+cishu+"，经验："+80*cishu+"，潜能："+40*cishu+"，体会："+20*cishu+"。\n",ctime(time()),me->query("name"),me->query("id")));	
	return 1;
}
