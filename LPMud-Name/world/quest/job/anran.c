...
mapping *quest = ({
  (["name":                "hgg",
    ...
    "place" :              "嘉峪关", ]),
  (["name":                "lt",
    "start" :              "/d/city3/nanheqiaos",
    "place" :              "成都南河桥",]),
  (["name":                "qc",
    "start" :              "/d/huashan/qianchi",
    "place" :              "华山千尺幢",]),
  (["name":                "hs",
    "start" :              "/d/fuzhou/zhongzhou",
    "place" :              "福州中洲桥",]),
  (["name":                "xz",
    "start" :              "/d/foshan/road8",
    "place" :              "佛山林间路",]),
});
string ask_job();
...
void create()
{
        set_name(HIR"马仇"NOR, ({ "ma chou", "ma"}) );
        set("gender", "男性" );
        set("nickname", "土匪头子");
        set("title", HIR"土匪"NOR);
        set("age", 31);
        set("no_suck", 1);
        set("no_get", 1);
        set("long", "这是土匪马仇。\n");
...
void destroying(object obj, object ob, object me)
{
   ...
   command("pat "+me->query("id"));
   message_vision(HIY"$N对$n说道：你这次做得极为出色，这是你该得的部分，拿去吧！说着拿出一些黄金给你！\n"NOR,ob, me);
   n_money = new("/clone/money/gold");
   n_money->move(me);
   n_money->add_amount(random(5));
   me->set_temp("robjob_exp", 500+(me->query_temp("biaoshi")*(random(300)+300)));
   me->add("combat_exp", me->query_temp("robjob_exp"));
   me->set_temp("biaoshi",me->query_temp("robjob_exp")-500);
   me->add("potential",me->query_temp("biaoshi"));
   tell_room(environment(me), me->name()+"数着钱袋里的金子，脸上的肌肉不住地颤动，眼睛散发着一种奇怪的光芒。\n" NOR, ({ me }));
   tell_object(me, RED"你被奖励了"+chinese_number(me->query_temp("robjob_exp"))+"点经验！   "+chinese_number(me->query_temp("biaoshi"))+"点潜能\n"NOR);
...
string ask_job()
{
  ...
  exp = me->query("combat_exp");
   ...
   // return "哼！看来你的心肠还不够黑啊，这种事你不问也罢。";
    ...
    if(exp>=6000000) j=6;
    else if(exp>=5000000) j=5;
    else if(exp>=4000000) j=5;
    else if(exp>=3000000) j=4;
    else j=3;
...
int wait_target(object ob, object me)
{
   ...
   if(!objectp(ob = present("ma chou", environment(me)))){
    ...
    message_vision(HIY"$N对着$n大声命令道：你快抢"+HIR+ZJSIZE(22)+ZJURL("cmds:qiang")+"抢(qiang)"+NOR+"镖队，我来对付其他人！\n"NOR, ob, me);
