...
varargs int do_uses(string arg)
{
	...
	write(INPUTTXT(HIR"注：值必须在13-30之间"ZJBR"总和不能超过你现有的先天"+max_sx+"点。"+NOR+ZJBR+HIW+"(臂力 悟性 根骨 身法)"+NOR+ZJBR+HIY+"(例：20 20 20 20，中间以空格隔开)"+NOR+ZJBR+"请输入四项先天点数：","xidian $txt#")+"\n");
	} else if(sscanf(arg, "-yes %d %d %d %d", tmpstr, tmpint, tmpcon, tmpdex) == 4) {
	if (tmpstr < 13 || tmpstr >30|| tmpint < 13 || tmpint >30|| tmpcon < 13 || tmpcon >30|| tmpdex < 13 || tmpdex >30) {
